...
void drawContour(const MSER::Region & region, int width, int height, int depth,
                 vector<uint8_t> & bits, const uint8_t * color){
    for (int i=0;i<region.pixellist.size();i++){
        int x=region.pixellist[i][0];
        int y=region.pixellist[i][1];
        for(int rgb=0;rgb<3;rgb++)
        bits[(y * width+x)*3+rgb] = color[rgb];
    }
...
int main(int argc, const char * argv[])
{
    ...
    MSER mser8(1, 0.005, 0.1, 0.05, 0.6, false);
    MSER mser4(1, 0.005, 0.1, 0.05, 0.6, false);
   ...
   // const uint8_t colors[2][3] = {{127, 127, 127}, {255, 255, 255}};
    vector<uint8_t> output_image(width * height*3);
	for (int i = 0; i < 2; ++i)
        for (int j = 0; j < regions[i].size(); ++j){
            const uint8_t colors[3]={ rand()%(255 + 1),rand()%(255 + 1),rand()%(255 + 1)};
            drawContour(regions[i][j], width, height, depth, output_image, colors);
        }
	
	// Save the original image
    saveJpeg(argv[2], width, height, 3, output_image);
